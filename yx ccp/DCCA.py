...
def load_image_data_as_samples(
    ...
    加载图像数据, 在初始阶段将每张图当作一条样本, 得到:
      X1: RGB histogram 特征 => shape = (dim1, n_samples) （以特征为行）
      X2: Gray histogram 特征 => shape = (dim2, n_samples) （以特征为行）
    ...
    X1 = np.array(X1_list, dtype=np.float32).T  # shape=(dim1, n_samples)
    X2 = np.array(X2_list, dtype=np.float32).T  # shape=(dim2, n_samples)
...
def train_test_split_on_samples(X1, X2, labels, test_size, random_state=42):
    ...
    return X1_tr.T, X1_te.T, X2_tr.T, X2_te.T, y_tr, y_te  # 返回 (dim1, n_samples), (dim2, n_samples), (n_samples,)
...
def evaluate_dcca_nn(
    ...
    """
    input_dim1 = X1_train.shape[0]  # 特征维度 1
    input_dim2 = X2_train.shape[0]  # 特征维度 2
    ...
    for n_comp in n_components_vals:
        for s in sigma_vals:  # 这里 sigma 在 DCCA 中没有使用
            if verbose:
            ...
            dcca.fit(X1_train, X2_train, epochs=20)  # 减少训练轮数以加快调试速度

            Z1_train = dcca.transform(X1_train, view=1)
            Z2_train = dcca.transform(X2_train, view=2)
            X_train_dcca = np.hstack([Z1_train, Z2_train])

            Z1_test = dcca.transform(X1_test, view=1)
            Z2_test = dcca.transform(X2_test, view=2)
...
def main():
    ...
    # 无需进一步转置，因为 load_image_data_as_samples 和 train_test_split_on_samples 已经进行了转置
    X1_train = X1_tr_samp
    X2_train = X2_tr_samp
    X1_test = X1_te_samp
    X2_test = X2_te_samp
    ...
    sigma_vals = [0.1, 1.0, 10.0]  # 注意: sigma 在 DCCA 中没有用到，这里保留是为了代码结构一致
